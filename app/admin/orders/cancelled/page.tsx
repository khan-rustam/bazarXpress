...
import { Eye, Edit, X, Search } from "lucide-react"
...
const mockOrders = [
  ...
  // ...add more demo cancelled orders as needed
]
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-codGray">Cancelled Orders</h2>
        </div>
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="py-4 px-6">
                      <div className={`flex items-center space-x-2 px-3 py-1 rounded-full ${statusConfig.cancelled.bg} w-fit`}>
                        <X className={`h-4 w-4 ${statusConfig.cancelled.color}`} />
                        <span className={`text-sm font-medium ${statusConfig.cancelled.color} capitalize`}>{order.status}</span>
                      </div>
                    </td>
